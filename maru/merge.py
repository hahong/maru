...
from .common import parse_opts_adapter, sort_uniq, \
        prep_files, set_new_threshold
...
Deprecated options:
   --filter_inf=<.inf prefix>    - reject any spikes not in the inf files
   --cluster=<.inf/.clu prefix>  - apply clustering information
   --cluster_all=<.clu prefix 1,.clu prefix 2,...>
                                 - use other clustering info (in this superset)

   Adding spike sorting information into the merged mwk files is now
   deprecated and potentially unreliable.
"""
...
def get_standard_cid(clu, tclu, nmax=np.inf, _debug=False):
    def sorted_cid(clu):
        ...
        for cid in clu:
            cluc[cid] += 1
        # sort
        return sorted([(cluc[cid], cid) for cid in cluc],
                reverse=True), len(cluc)

    # -- load other info
    cluu = list(set(list(clu)))     # unique cluster id
    nclu = len(cluu)
    # -- if there're too many clusters, coalesce small clusters into one
    # cid of "unsorted" group (default = -1, means no unsorted group)
    ucid = -1
    if nclu > nmax:
        clus, _ = sorted_cid(clu)
        # ccid: clusters to coalesce
        ccid = hash_tbl([cid for _, cid in clus[nmax - 1:]])
        # ucid: now we have the unsorted group. assign the
        # cid of "unsorted" group
        ucid = nclu + 1
    ...
    ncid = {}
    for i, (_, ocid) in enumerate(clus):
        ncid[ocid] = i
    for i in xrange(len(tclu)):
        tclu[i] = ncid[tclu[i]]
    # if there's "unsorted" group, get the new cid of it.
    if ucid >= 0:
        ucid = ncid[ucid]
...
def reject_inf(wav_info, ts):
    ...
    if ch not in reject_inf.valid:
        try:
            ...
            ninf1 = len(inf)
            if ninf1 != ninf0:
                print 'Ch:', ch, '\tchange in size: %d -> %d' % (ninf0, ninf1)
...
def cluster(wav_info, ts):
    ...
    if ch not in cluster.valid:
        try:
            ...
            inf = np.loadtxt(cluster.prefix_inf + '.inf.' + str(ch),
                    skiprows=1)
            clu = np.loadtxt(cluster.prefix_clu + '.clu.' + str(ch),
            ...
            if cluster.prefix_clu_all is None:
                clu_ref = clu
            else:
                clu_ref = load_superset(cluster.prefix_clu,
                        cluster.prefix_clu_all, ch)
...
# cluster.prefix_clu_all: prefix of all .clu files in this superset
# cluster.valid: valid positions
# cluster.clu: cluster id
# cluster.nclu: number of clusters
# cluster.nbad: number of bad ISIs
# cluster.nspk: number of spikes in this cluster
# cluster.ucid: cluster id of "unsorted" group (-1 if there's no such group)
# cluster.nmax: maximum units/ch
cluster.prefix_inf = ''
cluster.prefix_clu = ''
cluster.prefix_clu_all = None
cluster.valid = {}
cluster.clu = {}
cluster.nclu = {}
cluster.nbad = {}
...
def main(args):
    ...
    if 'cluster' in opts:
        ...
        if 'cluster_all' in opts:
            clu_all = prep_files(opts['cluster_all'],
                    sep=CLU_SEP, extchk=False)
    ...
    if m.merge(proc_wav=proc_wav,
            ...
            callback_cluster=callback_cluster,
