...
def tinfo2feat(ifn, ofn, tbs, tes, nooversized=True, preservefmt=True):
    ...
    if nooversized:
        idx2lbl = [(tb, te) for tb, te in zip(tbs, tes)
                if tb >= tmin and te <= tmax]
    else:
        idx2lbl = zip(tbs, tes)
    ...
    if preservefmt:
        ...
        orgfile = h5o.createCArray(meta, 'orgfile', atomi16,
                (n_lbl, n_tr, n_img, n_elec), filters=filters)   # origin info
